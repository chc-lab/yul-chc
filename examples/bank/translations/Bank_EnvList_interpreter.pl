...
%exec(Name, List) :-
    ...
    Cf1 = cf(cmd(Name, 'subO_external_fun_withdraw_84_Block2_3', fun_call(subO_fun_withdraw_84, [num(10)], [])), EnvList),


    %Cf1 = cf(cmd(Name, 'subO_external_fun_withdraw_42_Block2_3', fun_call(subO_fun_withdraw_42, [num(10)], [])), EnvList),        %bank yul v1 p1

	%Cf1 = cf(cmd(Name, 'subO_external_fun_deposit_44_Block0_3', fun_call(subO_fun_deposit_44, [], [])), EnvList),               %bank sol v1 p2
    %Cf1 = cf(cmd(Name, 'subO_external_fun_deposit_28_Block0_3', fun_call(subO_fun_deposit_28, [], [])), EnvList),               %bank yul v1 p2
    ...
    L1 = 'subO_external_fun_withdraw_84_Block2_4',

    %L1 = 'subO_external_fun_withdraw_42_Block2_3',                                                  %bank yul v1 p1
    
	%L1 = 'subO_external_fun_deposit_44_Block0_4',                                                   %bank  sol v1 p2
	%L1 = 'subO_external_fun_deposit_28_Block0_4',                                                   %bank yul v1 p2
...
updateevm_(Env2, Envf) :- 
    ...
    update(Env6,'calldata', V22 ,Env9),
     pl_ops((V00 is 80)), 
        update(Env9, off(0x00), V00 , Env10), 		%impostiamo balance_a = 80
    pl_ops((V01 is 0x123abc)), 
        update(Env10, off(0x01), V01 , Envf). 		%impostiamo address a  = 0x123abc
...
do_ex_call(Env0, R0, Gas, Wei, Envf):-
    ...
    nextlab(Con, L, L1),
    at(Con, L1, C),
    writeln(['entra nel caso di chiamata non riuscita']).
...
do_ex_call(Env0, R0, Gas, Wei, Envf):-
    ...
    nextlab(Con, L, L1),
    at(Con, L1, C),
    writeln(['entra nel caso di chiamata riuscita']).
