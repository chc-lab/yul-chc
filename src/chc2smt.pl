...
map2(smtInt,Cls) :-
  ...
  cls_to_hdbdpair(Cls,Cls1),
  assert((impl_spaces :- write('    '))),
  usmt_rules(Cls1,Rules,UsedTypes),
  sortByDependency(UsedTypes,SUsedTypes),
  % 1) declarations
  % 1.1) set-logic
  (smt_version(cvc4) ->
...
% where Vars is the list of variables occurring in Rule, and UsedTypes
% is the list of types occurring in Smt
usmt_rules(Cls,Rules,UsedTypes) :-
  usmt_rules(Cls,Rules,[],UsedTypes).
%
usmt_rules([],[],UsedTypes,UsedTypes).
usmt_rules([Cl|Cls],[(Vars,Rule)|Rules],UsedTypesIn,UsedTypesOut) :-
  usmt_rule(Cl,Rule,Vars),
  numbervars(Vars,0,_),
  usmt_rules(Cls,Rules,UsedTypesIn,UsedTypesOut).

%
usmt_rule(Cl,Cl1,Vars) :-
  Cl = (H,B),
  map_term_variables([H|B],Vars),
  conj_to_list(B,T),
  Cl1 = (H,T).
...
write_usmt_rules([]).
% fact
write_usmt_rules([Rule|Rules]) :-
  Rule=([],Cl),
  !,
  write('(assert'), nl,
  write_usmt_rule(Cl), nl,
  write(')'),        nl,
  write_usmt_rules(Rules).
% implication
write_usmt_rules([Rule|Rules]) :-
  write('(assert'), nl,
  Rule=(Vars,Cl),
  write('  (forall ( '), usmt_vars(Vars), write(')'), nl,
  write_usmt_rule(Cl), nl,
  write('  )'), nl,
  write(')'),   nl,
  write_usmt_rules(Rules).
...
write_usmt_rule(Cl) :-
  Cl=(H,B),
  impl_spaces, write('(=>'), nl,
  body_spaces, write('(and'), utermList2smt(B), nl, write('      )'), nl,
  head_spaces, uterm2smt(H), nl,
  impl_spaces, write(')').
...
uterm2smt(Term) :-
  ...
  memberchk(Pred,['=','=:=','=<','>=','>','<','-','/']),
  !,
  clp_to_smt(_Type,Pred,Pred1),
  write('('), write(Pred1), write(' '),
  uterm2smt(Lhs), write(' '), uterm2smt(Rhs),
  write(')').
uterm2smt(Term) :-
  Term =.. [Pred,Lhs,Rhs],
  memberchk(Pred,['+','*']),
...
uterm2smt(Term) :-
  ...
  !,
  NewT=..[=,Lhs,Rhs],
  write('(not '), uterm2smt(NewT), write(')').
% boolean connectives ----------------------------------------------------------
uterm2smt(Term) :-
  nonvar(Term),
  Term =.. [Op, Arg],
  memberchk(Op, ['not', '~']), 
  !,
  write('(not '), uterm2smt(Arg), write(')').
uterm2smt(Term) :-
  Term =.. [Pred,Lhs,Rhs],
  memberchk(Pred,['or','v',';']),
  !,
  write('(or '), uterm2smt(Lhs), write(' '), uterm2smt(Rhs), write(')').
uterm2smt(Term) :-
  Term =.. [Pred,Lhs,Rhs],
  memberchk(Pred,['and','&',',']),
  !,
  write('(and '), uterm2smt(Lhs), write(' '), uterm2smt(Rhs), write(')').
uterm2smt(Term) :-
  Term =.. ['=>',Lhs,Rhs],
  !,
  write('(=> '), uterm2smt(Lhs), write(' '), uterm2smt(Rhs), write(')').
% variables --------------------------------------------------------------------
uterm2smt(Term) :-
  Term=..[-,Var], 
  Var='$VAR'(_),
